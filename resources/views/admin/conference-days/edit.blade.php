...
@section('title', 'Edit Conference Day')
...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="mb-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between">
            <div>
                <h1 class="text-2xl md:text-3xl font-bold text-[#041E42]">Edit Conference Day</h1>
                <p class="text-gray-600 mt-1">Update attendance day information</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden border border-gray-200 max-w-3xl mx-auto">
        ...
        <form action="{{ route('admin.conference-days.update', $day['id']) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $day['name']) }}" 
                    ...
                    <div>
                        ...
                        <input type="date" name="date" id="date" value="{{ old('date', $day['date']) }}" 
                               class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#041E42]"
                               required>
                        @error('date')
                            <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3" 
                           class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#041E42]"
                           placeholder="Describe this conference day">{{ old('description', $day['description']) }}</textarea>
                    @error('description')
                        <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                    @enderror
                </div>
